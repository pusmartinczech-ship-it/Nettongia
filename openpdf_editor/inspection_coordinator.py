from __future__ import annotations

import shutil
import sys
import tempfile
from dataclasses import dataclass
from pathlib import Path

from PySide6.QtCore import QObject, QProcess, QTimer, Signal

from .inspection_worker import (
    DocumentInspectionReport,
    prepare_inspection_job,
    read_inspection_result,
)


@dataclass(frozen=True)
class InspectionContext:
    """Identity of the document snapshot inspected by a child process."""

    document_generation: int


@dataclass(frozen=True)
class InspectionOutcome:
    context: InspectionContext
    report: DocumentInspectionReport | None = None
    error: str | None = None


class InspectionCoordinator(QObject):
    """Own an isolated compatibility-inspection process and its timeout."""

    completed = Signal(object)

    def __init__(self, parent: QObject | None = None, *, timeout_ms: int = 45_000) -> None:
        super().__init__(parent)
        self._timeout_ms = timeout_ms
        self._request_id = 0
        self._process: QProcess | None = None
        self._workspace: Path | None = None
        self._result_path: Path | None = None
        self._context: InspectionContext | None = None
        self._forced_error: str | None = None
        self._timeout = QTimer(self)
        self._timeout.setSingleShot(True)
        self._timeout.timeout.connect(self._timed_out)

    @property
    def process(self) -> QProcess | None:
        return self._process

    @property
    def is_running(self) -> bool:
        return self._process is not None

    def start(
        self,
        source_bytes: bytes,
        *,
        encrypted_source: bool,
        context: InspectionContext,
    ) -> None:
        self.cancel()
        workspace = Path(tempfile.mkdtemp(prefix="OpenPDFEditor-inspection-"))
        try:
            job_path, result_path = prepare_inspection_job(
                workspace,
                source_bytes,
                encrypted_source=encrypted_source,
            )
        except Exception:
            shutil.rmtree(workspace, ignore_errors=True)
            raise

        self._request_id += 1
        request_id = self._request_id
        process = QProcess(self)
        process.setProcessChannelMode(QProcess.MergedChannels)
        if getattr(sys, "frozen", False):
            arguments = ["--document-inspection-worker", str(job_path)]
        else:
            launcher = Path(__file__).resolve().parents[1] / "run_editor.py"
            arguments = [str(launcher), "--document-inspection-worker", str(job_path)]
        process.setProgram(sys.executable)
        process.setArguments(arguments)
        process.finished.connect(
            lambda exit_code, exit_status, rid=request_id: self._process_finished(
                rid, exit_code, exit_status
            )
        )
        process.errorOccurred.connect(
            lambda process_error, rid=request_id: self._process_error(
                rid, process_error
            )
        )
        self._process = process
        self._workspace = workspace
        self._result_path = result_path
        self._context = context
        self._forced_error = None
        self._timeout.start(self._timeout_ms)
        process.start()

    def cancel(self) -> None:
        self._stop_timeout()
        self._request_id += 1
        process = self._process
        try:
            if process is not None and process.state() != QProcess.NotRunning:
                process.kill()
                process.waitForFinished(3000)
        except RuntimeError:
            pass
        self._cleanup()

    def _timed_out(self) -> None:
        process = self._process
        if process is None:
            return
        self._forced_error = "timeout"
        try:
            process.kill()
        except RuntimeError:
            self._complete(InspectionOutcome(self._require_context(), error="timeout"))

    def _process_error(
        self,
        request_id: int,
        _process_error: QProcess.ProcessError,
    ) -> None:
        process = self._process
        if request_id != self._request_id or process is None:
            return
        if process.state() == QProcess.NotRunning:
            self._process_finished(request_id, -1, QProcess.CrashExit)

    def _process_finished(
        self,
        request_id: int,
        exit_code: int,
        exit_status: QProcess.ExitStatus,
    ) -> None:
        process = self._process
        if request_id != self._request_id or process is None:
            return
        self._request_id += 1
        report: DocumentInspectionReport | None = None
        error = self._forced_error
        if error is None and exit_code == 0 and exit_status == QProcess.NormalExit:
            try:
                if self._result_path is None or not self._result_path.is_file():
                    raise ValueError("missing result")
                report, error = read_inspection_result(self._result_path)
            except Exception as exc:
                error = str(exc)
        elif error is None:
            output = bytes(process.readAllStandardOutput()).decode(
                "utf-8", errors="replace"
            ).strip()
            error = output or f"process exit {exit_code}"
        self._complete(
            InspectionOutcome(self._require_context(), report=report, error=error)
        )

    def _require_context(self) -> InspectionContext:
        if self._context is None:
            raise RuntimeError("The inspection context is missing.")
        return self._context

    def _complete(self, outcome: InspectionOutcome) -> None:
        self._stop_timeout()
        self._cleanup()
        self.completed.emit(outcome)

    def _stop_timeout(self) -> None:
        try:
            self._timeout.stop()
        except RuntimeError:
            pass

    def _cleanup(self) -> None:
        process = self._process
        workspace = self._workspace
        self._process = None
        self._workspace = None
        self._result_path = None
        self._context = None
        self._forced_error = None
        if process is not None:
            try:
                # Release native pipe/process handles before removing the
                # workspace.  Windows otherwise intermittently keeps a job or
                # result file locked until the deferred QObject destruction,
                # leaving an inspection directory behind after cancellation.
                process.close()
                process.deleteLater()
            except RuntimeError:
                pass
        if workspace is not None:
            shutil.rmtree(workspace, ignore_errors=True)

