# OpenPDF Editor 0.12.0 — public PDF corpus audit
...
document cannot be edited, so version 0.12.0 rejects it before thumbnail, outline,
or page-rendering code is entered.

Two encrypted documents are included in the sample. Both completed the audit with
their passwords supplied through the audit environment; the password values are not
...
ordinary office documents. The selection is deterministic, and every result in the
JSON report records the repository, commit, and repository-relative path.
...
Peak recorded child-process memory was 85,456 KiB and the slowest file completed in
1.023 seconds on the audit machine.

The complete per-file evidence is in `pdf_corpus_audit_0.12.0.json`. The reusable
